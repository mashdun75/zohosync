...
class Zoho_API {
    ...
    public function get_auth_url() {
        ...
        $scope = 'ZohoCRM.modules.ALL,ZohoCRM.settings.ALL,Desk.tickets.ALL,Desk.settings.ALL,Desk.search.READ,Desk.contacts.ALL';
    ...
    public function handle_oauth_callback($code) {
        ...
        // Make sure we're not using an empty or malformed code
        if (empty($code) || strlen($code) < 10) {
            error_log('Zoho OAuth Debug: Code appears to be invalid (empty or too short)');
            return array(
                'success' => false,
                'message' => 'OAuth failed: Invalid authorization code received'
            );
        }
        ...
        // Create a debug version with the secret masked
        $debug_data = $post_data;
        $debug_data['client_secret'] = '***MASKED***';
        $debug_data['code'] = substr($code, 0, 10) . '...' . substr($code, -5);
        error_log('Zoho OAuth Debug: Token request data: ' . print_r($debug_data, true));
        
        $response = wp_remote_post('https://accounts.zoho.com/oauth/v2/token', array(
            'body' => $post_data,
            'timeout' => 30 // Increase timeout for token requests
        ));
        ...
        if (!empty($data['access_token'])) {
            ...
            error_log('Zoho OAuth Debug: Failed to obtain token. Error: ' . $error);
            
            // Special handling for invalid_code error
            if ($error === 'invalid_code') {
                // The code was already used or expired - this is common if the page reloads or
                // the user clicks back/forward during the authentication flow
                error_log('Zoho OAuth Debug: Invalid code error received. This typically means the code was already used or expired.');
                return array(
                    'success' => false,
                    'message' => 'OAuth failed: The authorization code was invalid or expired. Please try connecting again.',
    ...
    /**
     * Get the proper domain for Desk API calls
     * 
     * @return string The API domain with www. removed if present
     */
    public function get_desk_api_domain() {
        $domain = $this->api_domain;
        // Make sure to remove www. for Desk API calls
