...
class GF_Zoho_Direct {
    ...
    /**
     * Process form submission to send data to Zoho
     */
    public static function process_submission($entry, $form) {
        ...
        if (empty($mappings) || empty($mappings['module']) || empty($mappings['fields'])) {
            error_log('Zoho Sync: No mappings found for form ' . $form['id']);
            return;
        }
        
        error_log('Zoho Sync: Processing submission for form ' . $form['id'] . ' with module ' . $mappings['module']);
        
        // Check if Zoho_API class exists
        if (!class_exists('Zoho_API')) {
            error_log('Zoho Sync: Zoho_API class not found');
            return;
        }
        ...
        if (!$api->get_access_token()) {
            error_log('Zoho Sync: Not connected to Zoho');
            return;
        }
        ...
        foreach ($mappings['fields'] as $gf_field => $zoho_field) {
            ...
            if ($gf_field === 'entry_id') {
                $value = $entry['id'];
                error_log("Zoho Sync: Using entry ID {$value} for Zoho field {$zoho_field}");
            } else {
                // Normal form field
            ...
            error_log("Zoho Sync: Mapping " . ($gf_field === 'entry_id' ? 'entry ID' : "form field $gf_field") . " to Zoho field $zoho_field with value: $value");
        }
        
        if (empty($data)) {
            error_log('Zoho Sync: No data to send to Zoho');
            return;
        }
        ...
        if (!empty($mappings['lookup_field']) && !empty($mappings['lookup_value'])) {
            ...
            if ($lookup_value_field === 'entry_id') {
                ...
                error_log("Zoho Sync: Looking up record with $lookup_field = $lookup_value (Entry ID)");
            } else {
                $lookup_value = rgar($entry, $lookup_value_field);
                error_log("Zoho Sync: Looking up record with $lookup_field = $lookup_value (Form field)");
            }
            ...
            if (!empty($lookup_value)) {
                ...
                if (!is_wp_error($response)) {
                    ...
                    if ($status === 200 && !empty($body['data'])) {
                        ...
                        error_log("Zoho Sync: Found existing record with ID: $record_id");
                    } else {
                        error_log("Zoho Sync: No existing record found with $lookup_field = $lookup_value");
                        error_log("Zoho Sync: Response: " . wp_remote_retrieve_body($response));
                    }
                } else {
                    error_log("Zoho Sync: Error looking up record: " . $response->get_error_message());
                }
            }
        }
        ...
        if ($record_id) {
            ...
            error_log("Zoho Sync: Updating record $record_id in {$mappings['module']}");
        } else {
            // Create new record
            ...
            error_log("Zoho Sync: Creating new record in {$mappings['module']}");
        }
        ...
        if (is_wp_error($response)) {
            error_log('Zoho Sync: Error sending data to Zoho: ' . $response->get_error_message());
            return;
        }
        ...
        if ($status === 200 || $status === 201 || $status === 202) {
            error_log('Zoho Sync: Data sent successfully to Zoho');
            
            // Store the Zoho record ID with the entry if created
            if (!empty($body['data']) && !empty($body['data'][0]['details']['id'])) {
                $zoho_id = $body['data'][0]['details']['id'];
                error_log("Zoho Sync: Record created with ID: $zoho_id");
            ...
            error_log('Zoho Sync: Error response from Zoho: ' . wp_remote_retrieve_body($response));
        }
    }
    ...
    public static function ajax_get_zoho_fields() {
        ...
        // Debug info
        error_log('Zoho Sync Debug: Getting fields for module: ' . $module);
        
        // Check if Zoho_API class exists
        if (!class_exists('Zoho_API')) {
            error_log('Zoho Sync Debug: Zoho_API class not found');
        ...
        if (!$access_token) {
            error_log('Zoho Sync Debug: Not connected to Zoho - no access token');
            wp_send_json_error('Not connected to Zoho');
        }
        
        // Log the token and API domain
        error_log('Zoho Sync Debug: Using access token: ' . substr($access_token, 0, 10) . '...');
        error_log('Zoho Sync Debug: Using API domain: ' . $api->api_domain);
        
        // Get fields for module
        $url = "https://{$api->api_domain}/crm/v2/settings/fields?module=" . urlencode($module);
        error_log('Zoho Sync Debug: Requesting URL: ' . $url);
        ...
        if (is_wp_error($response)) {
            ...
            error_log('Zoho Sync Debug: WP Error fetching fields: ' . $error_message);
        ...
        error_log('Zoho Sync Debug: Response status: ' . $status);
        error_log('Zoho Sync Debug: Response body: ' . substr($body, 0, 200) . '...');
        
        $body_data = json_decode($body, true);
        
        if ($status !== 200 || empty($body_data['fields'])) {
            $error_msg = isset($body_data['message']) ? $body_data['message'] : 'Unknown error';
            error_log('Zoho Sync Debug: Error response from Zoho: ' . $error_msg);
        ...
        error_log('Zoho Sync Debug: Found ' . count($fields) . ' fields for module: ' . $module);
    ...
    private static function render_forms_list() {
        ...
        <div class="wrap">
            ...
            <table class="widefat fixed striped" style="margin-top:20px;">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?php echo $has_mappings ? '✅ Mapped to ' . esc_html($mappings['module']) : 'Not configured'; ?></td>
                        <td>
                            <a href="?page=gf_zoho_sync&id=<?php echo $form['id']; ?>" class="button-secondary"><?php echo $has_mappings ? 'Edit' : 'Configure'; ?></a>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            ...
            <div style="background: #fff; padding: 15px; border: 1px solid #e5e5e5; margin-top: 20px;">
                ...
                <p>This plugin allows you to sync form submissions with Zoho CRM.</p>
                <p>To get started:</p>
                <ol>
                    ...
                    <li>Map form fields to Zoho CRM fields</li>
                </ol>
            </div>
        </div>
        <?php
    ...
    private static function render_form_page($form_id) {
        ...
        <div class="wrap">
            ...
            <form method="post" action="">
                ...
                <div style="background: #fff; padding: 15px; border: 1px solid #e5e5e5; margin-top: 20px;">
                    <h3>Zoho Module</h3>
                    <p>Select which Zoho CRM module to sync with:</p>
