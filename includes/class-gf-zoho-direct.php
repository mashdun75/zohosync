...
class GF_Zoho_Direct {
   ...
   public static function process_submission($entry, $form) {
    ...
    $record_id = null;
    // FIX: Add detailed logging about lookup field configuration
    $has_lookup_config = !empty($mappings['lookup_field']) && !empty($mappings['lookup_value']);
    gf_zoho_logger()->info("Checking for lookup configuration: " . ($has_lookup_config ? "FOUND" : "NOT FOUND"), array(
        'lookup_field' => isset($mappings['lookup_field']) ? $mappings['lookup_field'] : 'NOT SET',
        'lookup_value' => isset($mappings['lookup_value']) ? $mappings['lookup_value'] : 'NOT SET'
    ));
    
    if ($has_lookup_config) {
        ...
        if ($lookup_value_field === 'entry_id') {
            ...
            gf_zoho_logger()->info("Looking up record with $lookup_field = $lookup_value (Form field: $lookup_value_field)");
        }
        ...
        if (!empty($lookup_value)) {
            ...
            $response = wp_remote_get($url, array(
                ...
                'timeout' => 20 // Increase timeout for lookup
            ));
            ...
            if (!is_wp_error($response)) {
                ...
                gf_zoho_logger()->info("Lookup response status: $status", array(
                    'response_body_sample' => substr($body, 0, 500) . (strlen($body) > 500 ? '...' : '')
                ));
                
                if ($status === 200) {
                    if ($is_desk_module) {
                        // Desk API response format
                        if (!empty($body_data['data'])) {
                            $record_id = $body_data['data'][0]['id'];
                            gf_zoho_logger()->info("Found existing Desk record with ID: $record_id");
                        } else {
                            gf_zoho_logger()->info("No existing Desk record found with $lookup_field = $lookup_value");
                        }
                    } else {
                        ...
                        if (!empty($body_data['data'])) {
                            ...
                            gf_zoho_logger()->info("No existing CRM record found with $lookup_field = $lookup_value");
                        }
                    }
                } else {
            ...
            gf_zoho_logger()->warning("Empty lookup value from field $lookup_value_field - cannot perform lookup");
        }
    } else {
        gf_zoho_logger()->info("No lookup configuration found - will create new record");
    }
    ...
    if ($is_desk_module) {
        ...
        gf_zoho_logger()->info("CRM API response status: $status", array(
            'response_body_sample' => substr($body, 0, 500) . (strlen($body) > 500 ? '...' : '')
        ));
        ...
        if ($status === 200 || $status === 201 || $status === 202) {
            ...
            if (!$record_id && !empty($body_data['data']) && !empty($body_data['data'][0]['details']['id'])) {
                $zoho_id = $body_data['data'][0]['details']['id'];
    ...
    public static function ajax_get_zoho_fields() {
        ...
        if ($is_desk_module && class_exists('GF_Zoho_Desk')) {
            ...
            // Enhance field data with more information
            foreach ($fields as &$field) {
                // Add field description if available
                $field['description'] = isset($field['description']) ? $field['description'] : '';
                
                // Add lookup module info if this is a lookup field
                if (isset($field['type']) && strpos($field['type'], 'lookup') !== false) {
                    $field['lookup_module'] = isset($field['lookup_module']) ? $field['lookup_module'] : 'Unknown';
                    $field['is_lookup'] = true;
                } else {
                    $field['is_lookup'] = false;
                }
            }
            ...
            foreach ($body_data['fields'] as $field) {
                ...
                // Add lookup module information if available
                if (isset($field['data_type']) && 
                    (strpos($field['data_type'], 'lookup') !== false || 
                     strpos($field['data_type'], 'Lookup') !== false)) {
                    $field_info['is_lookup'] = true;
                    $field_info['lookup_module'] = isset($field['lookup']['module']) ? $field['lookup']['module'] : 'Unknown';
                }
                ...
                function enhanceZohoFieldDisplay() {
                    ...
                    $('.zoho-field-select').each(function() {
                        ...
                        if (fieldName && window.zohoFieldInfo && window.zohoFieldInfo[fieldName]) {
                            ...
                            if (fieldDetails.type.includes('lookup') && !$select.next('.lookup-note').length) {
