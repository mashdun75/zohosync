...
class GF_Zoho_Logger {
     ...
     * Log a lookup action
     */
    public function lookup($message, $context = array()) {
        // Create a special lookup log level
        $this->log($message, 'LOOKUP', $context);
        
        // Also log to WordPress debug log if enabled
        if (defined('WP_DEBUG') && WP_DEBUG && defined('WP_DEBUG_LOG') && WP_DEBUG_LOG) {
            error_log("Zoho Lookup [{$message}]: " . json_encode($context));
        }
    }

    /**
     * Get lookup logs only
     *
     * @param int $lines Number of lines to retrieve
     * @return string Lookup log content
     */
    public function get_lookup_log_content($lines = 100) {
        if (!file_exists($this->log_file)) {
            return 'No log file found.';
        }
        
        // Get all log lines
        $all_lines = $this->get_log_content(1000); // Get more lines than needed to filter
        
        // Split into array
        $log_lines = explode("\n", $all_lines);
        
        // Filter for lookup entries only
        $lookup_lines = array();
        foreach ($log_lines as $line) {
            if (strpos($line, '[LOOKUP]') !== false) {
                $lookup_lines[] = $line;
            }
        }
        
        // Get only the requested number of lines (most recent first)
        $lookup_lines = array_slice(array_reverse($lookup_lines), 0, $lines);
        
        // Return as string (restored to chronological order)
        return implode("\n", array_reverse($lookup_lines));
    }
}
