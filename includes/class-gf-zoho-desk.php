...
class GF_Zoho_Desk {
    ...
    private function get_region() {
        ...
        $this->logger->info("Using Zoho API region: {$region} (from domain: {$api_domain})");
        return $region;
    }
    ...
    public function get_desk_fields($module) {
        ...
        $fields_url = "https://desk.zoho.{$region}/api/v1/fields?module={$module}&orgId={$this->organization_id}";
    ...
    public function submit_to_desk($module, $data, $record_id = null) {
        ...
        // Verify required fields based on module
        if ($module === 'tickets') {
            // Ticket requires subject and departmentId
            if (empty($data['subject'])) {
                $this->logger->error("Missing required field 'subject' for ticket creation");
                return array(
                    'success' => false,
                    'message' => "Error: 'subject' is required for ticket creation"
                );
            }
            
            if (empty($data['departmentId'])) {
                $this->logger->error("Missing required field 'departmentId' for ticket creation");
                return array(
                    'success' => false,
                    'message' => "Error: 'departmentId' is required for ticket creation"
                );
            }
            
            // Either contactId or contact object must be provided
            if (empty($data['contactId']) && empty($data['contact'])) {
                $this->logger->warning("Neither 'contactId' nor 'contact' provided for ticket creation. This may cause an error.");
            }
        }
        
        // Process lookup fields for Desk modules
        $data = $this->process_desk_lookup_fields($data, $module);
        ...
        $this->logger->info("Making {$method} request to URL: {$url}");
        ...
        $this->logger->debug("Response body: " . substr($body, 0, 500) . (strlen($body) > 500 ? '...' : ''));
    ...
    public function render_desk_settings_tab() {
        ...
        $scopes = urlencode('Desk.tickets.ALL,Desk.contacts.ALL,Desk.basic.READ,Desk.search.READ,Desk.settings.READ,Desk.settings.UPDATE');
        ...
        <ol>
            ...
            <li>Select at least these Zoho Desk scopes: 
                <ul>
                    ...
                    <li>Desk.settings.READ</li>
                    <li>Desk.settings.UPDATE</li>
                </ul>
            </li>
            ...
            <table class="form-table">
                <tr>
                    ...
                    <td>
                        ...
                        <p class="description">The department to assign tickets to. <strong>Required</strong> for ticket creation.</p>
                    </td>
                </tr>
                <tr>
    ...
    /**
     * Process Desk settings submission
     *
     * @param array $mappings Current mappings
    ...
    /**
     * Add UI for Desk settings in multi-module mappings
     *
...
// Include the Desk lookup field handling methods
require_once dirname(__FILE__) . '/desk-lookup-methods.php';
